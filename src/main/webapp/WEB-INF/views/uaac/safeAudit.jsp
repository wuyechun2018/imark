	...
	$(function(){
		...
		dgSafeAudit=$('#list_tb').datagrid({    
		    ...
		    columns:[[    
				...
				{field:'OPERATE_CONDITION',title:'操作内容',sortable:true,width:100,
                    formatter:function(val,rec,index){ 
                        return '<span data-p1='+index+' class="easyui-tooltip">' + val + "(" + rec.OPERATE_NAME + ")" + '</span>';  
                  }  
                }
		    ]],
		    //toolbar:'#tb',
	 ...
	 function createTooltip(){
          $('#list_tb').datagrid('getPanel').find('.easyui-tooltip').each(function(){
             var index = parseInt($(this).attr('data-p1')); 
             $(this).tooltip({  
                 content: $('<div></div>'),  
                 onUpdate: function(cc){  
                     var row = $('#list_tb').datagrid('getRows')[index];  
                     var content = '<div>'+ row.OPERATE_CONDITION+ "(" + row.OPERATE_NAME + ")" +'</div>'
                     cc.panel({  
                         width:200,  
                         content:content
                     });  
												...
												<tr>
													<td colspan="4" class="text-right">
														<a class="btn btn-primary" id="queryBtn" href="javascript:;"> 
															<span class="l"> </span><span id="search" class="c" style="color:#333"><i class="icon icon-query"></i>查询</span><span class="r"></span>
														</a>
													</td>
												</tr>
											</tbody>
